 ...
 *********************************************************************/
    data_t half_inner_contig(ParVector &x, ParVector &y, int half, int part_global) {
        ...
        if (!(color)) {
            ...
            if (inner_comm_size > 1)
                RAPtor_MPI_Allreduce(RAPtor_MPI_IN_PLACE, &inner_prod, 1, RAPtor_MPI_DATA_T, RAPtor_MPI_SUM,
                                     inner_comm);
        }
        ...
        if (rank == recv_root)
            RAPtor_MPI_Recv(&inner_prod, 1, RAPtor_MPI_DATA_T, inner_root, 1, RAPtor_MPI_COMM_WORLD,
 ...
 ****************************************************************/
    data_t sequential_inner(ParVector &x, ParVector &y) {
        ...
        if (rank > 1) {
            RAPtor_MPI_Recv(&inner_prod, 1, RAPtor_MPI_DATA_T, rank - 1, 1, RAPtor_MPI_COMM_WORLD,
                            RAPtor_MPI_STATUS_IGNORE);
        }
 ...
 ****************************************************************/
    ...
    void create_partial_inner_comm(RAPtor_MPI_Comm &inner_comm, RAPtor_MPI_Comm &root_comm, double frac, ParVector &x,
                                   int &my_inner_color,
 ...
 ****************************************************************/
    data_t
    half_inner(RAPtor_MPI_Comm &inner_comm, ParVector &x, ParVector &y, int &my_color, int send_color, int &inner_root,
 ...
 ****************************************************************/
    data_t
    partial_inner(RAPtor_MPI_Comm &inner_comm, RAPtor_MPI_Comm &root_comm, ParVector &x, ParVector &y, int my_color,
